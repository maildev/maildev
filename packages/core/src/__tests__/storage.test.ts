...
describe('MemoryStorage', () => {
  ...
  describe('listSummaries', () => {
    it('should omit bodies and headers but keep what a list renders', async () => {
      await storage.save(
        createTestEmail('1', {
          html: '<p>a very large body</p>',
          text: 'a very  large\n body',
          headers: { 'x-large-header': 'x'.repeat(1000) },
          attachments: [
            {
              filename: 'notes.txt',
      ...
      const result = await storage.listSummaries()
      ...
      expect(summary.subject).toBe('Test Subject 1')
      expect(summary.attachmentCount).toBe(1)
      // Whitespace collapsed so the preview renders on one line
      expect(summary.preview).toBe('a very large body')
    })

    it('should carry the same counts as list', async () => {
      await storage.save(createTestEmail('1'))
      await storage.save(createTestEmail('2', { read: true }))

      const result = await storage.listSummaries({ limit: 1 })
