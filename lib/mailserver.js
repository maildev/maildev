...
function saveEmail(id, envelope, mailObject){
  ...
  if (mailServer.autoRelay) {
    mailServer.relayEmailQueue.push({
      idOrMailObject:object,
      autoRelay:true,
  ...
  mailServer.getRawEmail(mail.id, function(err, rawEmailStream) {
    ...
    var mailConnectCallback = function(err, info) {
      ...
      var mailSendCallback = function(err, info) {
        ...
        var recipients = mail.envelope.to.map(getAddress);
        var sender = getAddress(mail.envelope.from);
...
function relayWorker(task, callback) {
  ...
  relayMail(task.idOrMailObject, task.autoRelay, relayCallback);
...
mailServer.create = function(port, host, user, password) {
  ...
  mailServer.relayEmailQueue = async.queue(relayWorker, 1);
};
...
mailServer.setAutoRelayMode = function(enabled, rules){
  ...
  if (mailServer.autoRelay) {
    logger.info(
      ...
      JSON.stringify(mailServer.autoRelayRules)
    );
  }
};
