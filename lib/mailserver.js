...
const groupHeaderName = 'x-maildev-group'
...
const getGroupFromEmail = function (email) {
  // eslint-disable-next-line no-prototype-builtins
  return email.headers.hasOwnProperty(groupHeaderName) ? email.headers[groupHeaderName] : defaultGroupName
}

/**
 * Get all groups
 */
mailServer.getAllGroup = function (done) {
  const groups = store
    .map(getGroupFromEmail)
...
/**
 * Get all emails by group
 */
mailServer.getAllEmailByGroup = function (group, done) {
  const filtered = store.filter(function (email) {
    return getGroupFromEmail(email) === group || group === defaultGroupName
  })
  done(null, filtered)
}

/**
 * Delete all Email by group
 */
mailServer.deleteEmailByGroup = function (group, done) {
  store.slice().reverse().forEach(function (email) {
    if (getGroupFromEmail(email) === group || group === defaultGroupName) {
      mailServer.deleteEmail(email.id, function () {})
    }
  })
  done(null, true)
}
