...
mailServer.setupForwarding = function(host, port, user, pass, secure){
  ...
  try {
    ...
    logger.info(
      'MailDev forwarding to %s:%d (user:%s, pass:%s, secure:%s)',
      host,
      port,
      user,
      pass ? '####' : pass,
...
mailServer.sendMail = function(mail){
  if (!mailServer.clientPool) return;

  mailServer.getRawEmail(mail.id, function(err, rawEmailStream){
    if (err) console.error('Mail Stream Error: ', err);
    ...
    mailServer.clientPool.sendMail(mailComposer, function(err, response){
      if (err) return console.error('Mail Delivery Error: ', err);
