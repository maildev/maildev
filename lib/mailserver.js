...
mailServer.listen = function (callback) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    mailServer.smtp.listen(mailServer.port, mailServer.host, function (err) {
      ...
      const printHost = host === '::' ? 'localhost' : mailServer.host;
      logger.info(
...
mailServer.off = eventEmitter.off.bind(eventEmitter)
mailServer.once = eventEmitter.once.bind(eventEmitter)
mailServer.prependListener = eventEmitter.once.bind(eventEmitter)
mailServer.prependOnceListener = eventEmitter.once.bind(eventEmitter)
...
mailServer.deleteEmail = function (id, done) {
  ...
  fs.unlink(path.join(mailServer.mailDir, id + '.eml'), function (err) {
    if (err) {
      ...
      eventEmitter.emit('delete', { id, index: emailIndex })
    }
  })

  if (email.attachments) {
    fs.rm(path.join(mailServer.mailDir, id), { recursive: true }, function (err) {
      if (err) throw err
    })
  }
