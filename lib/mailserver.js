...
function saveEmail (id, envelope, mailObject) {
  ...
  if (mailServer.mailLifeSpan > 0) {
    scheduler.resetTime(object.time.getTime() + mailServer.mailLifeSpan)
  }
}
...
/**
 * Deleting emails with out of life span
 */

function mailLifeSpanCheck () {
  const now = (new Date()).getTime()
  const deleteId = []

  for (let mail of store) {
    const mailTime = Date.parse(mail.time)
    const lifeTime = now - mailTime
    if (lifeTime < mailServer.mailLifeSpan) {
      scheduler.resetTime(mailTime + mailServer.mailLifeSpan)
      break
    }
    deleteId.push(mail.id)
  }

  deleteId.forEach(function (id) {
    mailServer.deleteEmail(id, function () {})
  })
}
...
mailServer.create = function (port, host, user, password, hideExtensions, mailLifeSpan) {
  ...
  mailServer.mailLifeSpan = mailLifeSpan ? mailLifeSpan * 1000 : 0
  if (mailServer.mailLifeSpan > 0) {
    scheduler.on('timeout', mailLifeSpanCheck)
  }
