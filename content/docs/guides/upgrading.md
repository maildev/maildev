...
description: Move a MailDev 2.x setup to 3.0 — what needs Node 20+, what changed for the CLI, the REST API, the programmatic API, and Docker, and what did not change at all.
ogTitle: Upgrading MailDev from 2.x to 3.0
permalink: /docs/guides/upgrading/
updated: 2026-09-23
---
...
[MCP server](/docs/ai/mcp/). "Did the signup email arrive?" becomes a step the
agent does instead of one you do — the
[Claude Code walkthrough](/docs/ai/claude-code/) shows the full loop.
:::
...
[Docker guide](/docs/guides/docker/) on the health check and volume mounting.
...
[configuration file](/docs/reference/cli/) is the better
...
| `--mcp` | `MAILDEV_MCP` | Serve the [MCP endpoint](/docs/ai/mcp/) at `/mcp` |
...
The [programmatic API reference](/docs/reference/node-api/) documents the full
...
[CI guide](/docs/guides/testing-in-ci/) is built around it.
:::
...
The [Docker guide](/docs/guides/docker/) covers Compose, the volume mount for
