...
function getAutoRelayableRecipients(recipients){
  ...
  for (var i= 0, len = recipients.length; i<len; i++){
    if (validateAutoRelayRules(recipients[i])){
      emails.push(recipients[i]);
    }
  }
  return emails;
}
...
function validateAutoRelayRules(email){
  ...
  var result = true;
  for (var i= 0, len= mailServer.autoRelayRules.length; i<len; i++){
    var rule = mailServer.autoRelayRules[i];

    if (wildstring.match((rule.allow || rule.deny), email)){
      result = rule.allow ? true : false;
    }
...
mailServer.setAutoRelayMode = function(enabled, rules){
  ...
  if (rules && rules instanceof Array){
    mailServer.autoRelayRules = include;
  }
  else if (rules && typeof rules === "string"){
    mailServer.autoRelayRules = JSON.parse(fs.readFileSync(rules, "utf8"));
  }
