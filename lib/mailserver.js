...
mailServer.onSmtpError = function(e) {
  logger.info("Could not start mail server on " + e.address + ":" + e.port + "\nPort already in use or insufficient rights to bind port");
...
mailServer.end = function(callback) {
  ...
  mailServer.smtp[method](callback);
  outgoing.close();
};
...
/**
 * Overrides default store with version that has limited capacity
 */

mailServer.setMessagesLimit = function(messagesLimit) {
  mailServer.store = store = storage;
  storage.setLimit(messagesLimit);

  store.on('delete', function(item){
    mailServer.deleteEmail(item.id, function(err, done){
      if(err){
        logger.error("Can't delete email due to: ", err);
      }
    });
  });
};
