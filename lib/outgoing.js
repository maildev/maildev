...
outgoing.setAutoRelayMode = function (enabled, rules, autoRelayAllToAddress) {
  if (!client) {
  ...
  if (autoRelayAllToAddress) {
    config.autoRelayAllToAddress = autoRelayAllToAddress
  }
  ...
  if (config.autoRelay) {
    logger.info(
      'Auto-Relay mode on, %srelay rules: %s',
      config.autoRelayAllToAddress
        ? 'relaying all messages to:[' + config.autoRelayAllToAddress.toString() + '], '
        : '',
...
function relayMail (emailObject, emailStream, isAutoRelay, done) {
  ...
  if (isAutoRelay && config.autoRelayAllToAddress) {
    emailObject.to = [{address: config.autoRelayAllToAddress}]
    emailObject.envelope.to = [{ address: config.autoRelayAllToAddress, args: false }]
  }
